...
description: "**PPT 한국형 디자인 시스템**: Noto Sans KR 기반 프로페셔널 타이포그라피, 6:3:1 색상 법칙, 한국 비즈니스 PPT 레이아웃 가이드라인을 적용합니다. PptxGenJS 기반 코드 생성 시 네이티브 요소(테이블·차트·이미지·도형·텍스트)를 목적에 맞게 매핑하여 최적 품질의 PPTX를 생성합니다.\n  - MANDATORY TRIGGERS: PPT 디자인, 슬라이드 디자인, 프레젠테이션 디자인, PPT 템플릿, 발표자료, 피치덱, pitch deck, PPT 폰트, 슬라이드 레이아웃"
---
...
Noto Sans KR 폰트 기반으로 전문적이고 가독성 높은 슬라이드를 생성합니다.
...
| 폰트 | 용도 | Weight |
|-------|------|--------|
| **Noto Sans KR** | 제목/본문/캡션/인용 등 전체 | 6종: Thin(100)~Black(900) |

### Noto Sans KR Weight 가이드

| Weight | 용도 |
|--------|------|
| Thin (100) | 장식용 대형 텍스트, 워터마크 |
| Light (300) | 긴 본문, 설명 텍스트, 캡션 |
| Regular (400) | 기본 본문 |
| Medium (500) | 강조 본문, 라벨, 소제목 |
| Bold (700) | 섹션 제목, 헤더, 메인 제목 |
| Black (900) | KPI 숫자, 히어로 텍스트 |

### 폰트 적용 규칙

```
제목(Title):     Noto Sans KR Bold / Black       36~44pt
소제목(Subtitle): Noto Sans KR Medium / Bold      24~28pt
본문(Body):      Noto Sans KR Regular / Medium    16~20pt
캡션(Caption):   Noto Sans KR Light / Regular     12~14pt
인용구(Quote):   Noto Sans KR Medium              18~22pt (격식체, 인용문)
숫자강조(KPI):   Noto Sans KR Black               48~72pt
장식텍스트:      Noto Sans KR Thin / Light        60~120pt (배경 대형 텍스트)
```
...
[상단: 제목 바]
[3~4열 KPI 카드]
  숫자: Noto Sans KR Black 48pt (accent 색상)
...
const CHART_STYLE = {
  ...
  base: {
    showTitle: true,
    titleFontFace: 'Noto Sans KR',
    titleFontSize: 14,
    titleColor: COLORS.text_primary,
    showLegend: true,
    legendFontFace: 'Noto Sans KR',
    legendFontSize: 9,
    legendColor: COLORS.text_secondary,
    catAxisLabelFontFace: 'Noto Sans KR',
    catAxisLabelFontSize: 10,
    catAxisLabelColor: COLORS.text_tertiary,
    valAxisLabelFontFace: 'Noto Sans KR',
...
슬라이드 내용에 시각화가 필요한 요소가 포함된 경우, **svg-diagram 스킬을 자동으로 호출**하여 도식을 생성하고 슬라이드에 삽입한다.

#### 자동 트리거 조건

| 슬라이드 내용 | 생성 도식 | svg-diagram 함수 |
|--------------|-----------|-------------------|
| 업무 프로세스, 승인 흐름 | 플로우차트 | Mermaid flowchart 또는 createProcessArrow() |
| 조직/팀 구조 | 조직도 | Mermaid graph TD 또는 createOrgChart() |
| KPI, 핵심 지표 | KPI 카드 | createKpiCards() |
| 일정, 마일스톤 | 타임라인 | createTimeline() |
| 비율, 구성비 | 도넛 차트 | createDonutChart() |
| 전후 비교 | Before/After | createComparison() |

#### 연계 워크플로우

```
슬라이드 내용 분석 → 도식 필요 여부 판단
→ svg-diagram 스킬 호출 → SVG 생성
→ sharp로 PNG 변환 → addImage()로 슬라이드에 삽입
```

```javascript
// 자동 연계 예시: KPI 슬라이드에 KPI 카드 도식 삽입
const svgString = createKpiCards([
  { label: '월 매출', value: '12.5억', change: '+8.2%' },
  { label: '신규 고객', value: '847명', change: '+12%' },
  { label: '고객 만족도', value: '94.2%', change: '+2.1%' },
]);
const pngBuffer = await sharp(Buffer.from(svgString)).png().toBuffer();
slide.addImage({
  data: 'image/png;base64,' + pngBuffer.toString('base64'),
  x: 0.6, y: 1.8, w: 12.13, h: 2.5
});
```

#### 연계 규칙

1. 슬라이드 제목/내용에서 위 조건에 해당하는 내용이 감지되면 자동으로 도식 생성
2. addChart()로 표현 가능한 단순 차트(막대, 꺾은선, 원형)는 네이티브 차트 우선 사용
3. addChart()로 표현 불가능한 복합 시각화(KPI 카드, 타임라인, 프로세스 화살표 등)는 svg-diagram 연계
4. 사용자가 "도식 없이"라고 요청한 경우 도식 생성을 생략
...
slide.addText('슬라이드 제목', {
  ...
  fontSize: 28, fontFace: 'Noto Sans KR', bold: true,
...
slide.addText([
  ...
  fontSize: 16, fontFace: 'Noto Sans KR',
  color: COLORS.text_secondary,
...
slide.addText('\u201C변화를 두려워하지 마세요\u201D', {
  x: 2, y: 3, w: 9, h: 1,
  fontSize: 22, fontFace: 'Noto Sans KR', italic: true,
...
// Noto Sans KR 폰트 사용:
const FONTS = {
  title:    { fontFace: 'Noto Sans KR', bold: true },   // ExtraBold/Black
  subtitle: { fontFace: 'Noto Sans KR', bold: true },   // SemiBold/Bold
  body:     { fontFace: 'Noto Sans KR', bold: false },   // Regular/Medium
  caption:  { fontFace: 'Noto Sans KR', bold: false },   // Light/Regular
  serif:    { fontFace: 'Noto Sans KR', bold: false }, // 인용구/격식
  kpi:      { fontFace: 'Noto Sans KR', bold: true },    // Black
  deco:     { fontFace: 'Noto Sans KR', bold: false },   // Thin/ExtraLight (장식용)
};
```
...
function addCard(slide, { x, y, w, h, title, body, accentColor }) {
  ...
  slide.addText(title, {
    ...
    fontSize: 16, fontFace: 'Noto Sans KR', bold: true,
    color: COLORS.text_primary
  });
  // 카드 본문
  slide.addText(body, {
    x: x + 0.2, y: y + 0.55, w: w - 0.4, h: h - 0.75,
    fontSize: 13, fontFace: 'Noto Sans KR',
    color: COLORS.text_secondary,
...
function addPageNumber(slide, num, total) {
  slide.addText(`${num} / ${total}`, {
    ...
    fontSize: 9, fontFace: 'Noto Sans KR',
...
- [ ] 여백: 좌우 0.6" / 상하 0.5" 이상
- [ ] 한 슬라이드 1 메시지 원칙
- [ ] 폰트 1종 통일 (Noto Sans KR)
- [ ] 한글 깨짐 없음 (Noto Sans KR 폰트 확인)
