---
name: svg-diagram
description: "**SVG 도식화 & Mermaid 변환기**: 비즈니스 문서용 SVG 다이어그램을 생성하고, Mermaid 구문을 테마 색상에 맞는 SVG로 변환합니다. 프로세스 플로우, 조직도, 타임라인, KPI 카드, 인포그래픽 등 문서 삽입용 시각자료를 생성합니다.\n  - MANDATORY TRIGGERS: 도식화, 다이어그램, SVG, 플로우차트, 조직도, 프로세스, 인포그래픽, mermaid, 시각화, 차트 이미지, 도표"
---
...
function createKpiCards(kpis, options = {}) {
  ...
  kpis.forEach((kpi, i) => {
    ...
    // 수치
    content += `<text x="${x + cardWidth/2}" y="80" text-anchor="middle" font-size="36" font-weight="800" fill="${color}" font-family="Noto Sans KR">${kpi.value}</text>`;
    // 변화량
    if (kpi.change) {
      const changeColor = kpi.change.startsWith('+') ? '#00D4AA' : '#FF6B6B';
      content += `<text x="${x + cardWidth/2}" y="110" text-anchor="middle" font-size="14" fill="${changeColor}" font-family="Noto Sans KR">${kpi.change}</text>`;
    }
  });
...
function createComparison(before, after, options = {}) {
  ...
  content += `<text x="${boxW/2 + 10}" y="${startY + 30}" text-anchor="middle" font-size="18" font-weight="700" fill="#FF6B6B" font-family="Noto Sans KR">BEFORE</text>`;
  
  // After 박스
  content += `<rect x="${width/2 + 20}" y="${startY}" width="${boxW}" height="${boxH}" rx="12" fill="#F0FFF4" stroke="#00D4AA" stroke-width="2"/>`;
  content += `<text x="${width/2 + 20 + boxW/2}" y="${startY + 30}" text-anchor="middle" font-size="18" font-weight="700" fill="#00D4AA" font-family="Noto Sans KR">AFTER</text>`;
...
function createOrgChart(data, options = {}) {
  ...
  function drawNode(x, y, label, level) {
    ...
    content += `<text x="${x + nodeW/2}" y="${y + nodeH/2 + 5}" text-anchor="middle" font-size="14" font-weight="${level < 2 ? 700 : 500}" fill="${textColor}" font-family="Noto Sans KR">${label}</text>`;
  }
...
function createTimeline(items, options = {}) {
  ...
  items.forEach((item, i) => {
    ...
    // 날짜 (위)
    content += `<text x="${x}" y="${lineY - 25}" text-anchor="middle" font-size="12" font-weight="600" fill="${color}" font-family="Noto Sans KR">${item.date}</text>`;

    // 이벤트 (아래)
    content += `<text x="${x}" y="${lineY + 35}" text-anchor="middle" font-size="13" fill="#4A5568" font-family="Noto Sans KR">${item.label}</text>`;
    if (item.desc) {
      content += `<text x="${x}" y="${lineY + 52}" text-anchor="middle" font-size="11" fill="#718096" font-family="Noto Sans KR">${item.desc}</text>`;
    }
  });
...
function createDonutChart(data, options = {}) {
  ...
  data.forEach((item, i) => {
    ...
    content += `<text x="${lx}" y="${ly}" text-anchor="middle" font-size="12" fill="#4A5568" font-family="Noto Sans KR">${item.label} ${pct}%</text>`;

    startAngle = endAngle;
  });

  // 중앙 텍스트
  content += `<text x="${cx}" y="${cy - 5}" text-anchor="middle" font-size="28" font-weight="800" fill="#1A1F36" font-family="Noto Sans KR">${total.toLocaleString()}</text>`;
  content += `<text x="${cx}" y="${cy + 18}" text-anchor="middle" font-size="12" fill="#718096" font-family="Noto Sans KR">합계</text>`;
...
- **한글 폰트**: SVG 내 `<style>`에 Noto Sans KR @import 포함 필수
- **PNG 변환 시**: `sharp`에 density 옵션으로 고해상도 확보 (150+ DPI)
- **Mermaid 한글**: fontFamily에 'Noto Sans KR' 지정
